...
export class CrossyRenderer extends WebGLRenderer {
  __gl: any;
  constructor(props) {
    super({
      canvas: props.canvas,
    ...
    this.__gl = props.gl ?? { endFrameEXP: () => {} };
...
export class GameMap {
  ...
  checkCollectible = (position) => {
    const targetZ = `${position.z | 0}`;
    if (targetZ in this.floorMap) {
      const { type, entity } = this.floorMap[targetZ];
      if (type === "grass" && entity.collectibleMap) {
        const key = `${position.x | 0}`;
        if (key in entity.collectibleMap) {
          const item = entity.collectibleMap[key];
          if (item && item.mesh) {
            entity.floor.remove(item.mesh);
            delete entity.collectibleMap[key];
            return item;
          }
        }
      }
  ...
  pullCollectibles = (position, radius = 5) => {
    const collected = [];
    for (const rowKey of Object.keys(this.floorMap)) {
      const row = this.floorMap[rowKey];
      if (!row || row.type !== "grass" || !row.entity.collectibleMap) {
        continue;
      }

      const rowZ = Number(rowKey);
      if (Math.abs(rowZ - position.z) > radius) {
        continue;
      }

      for (const itemKey of Object.keys(row.entity.collectibleMap)) {
        const item = row.entity.collectibleMap[itemKey];
        if (!item || !item.mesh) {
          continue;
        }

        const dx = position.x - item.mesh.position.x;
        const dz = position.z - rowZ;
        if (Math.abs(dx) > radius || Math.abs(dz) > radius) {
          continue;
        }

        item.mesh.position.x += dx * 0.2;
        item.mesh.position.z += dz * 0.2;

        if (Math.abs(dx) < 0.55 && Math.abs(dz) < 0.55) {
          row.entity.floor.remove(item.mesh);
          delete row.entity.collectibleMap[itemKey];
          collected.push(item);
        }
      }
    }
...
export class CrossyGameMap extends GameMap {
  ...
  tick(dt, hero, speedScale = 1) {
    for (const grass of this.grasses.items) {
      if (grass.update) grass.update(dt, hero);
    }
    for (const railRoad of this.railRoads.items) {
      railRoad.update(dt, hero, speedScale);
    }
    for (const road of this.roads.items) {
      road.update(dt, hero, speedScale);
    }
    for (const water of this.water.items) {
      water.update(dt, hero, speedScale);
    }
  }
