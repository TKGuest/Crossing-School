...
export default class Road extends Object3D {
  ...
  carGen = () => {
    ...
    for (let x = 0; x < numCars; x++) {
      if (this.cars.length - 1 < x) {
        ...
        const speedScale = mesh.userData?.speedScale || 1;
      ...
      this.cars[x].speed = speed * this.cars[x].speedScale * xDir;
  ...
  update = (dt, player, speedScale = 1) => {
    ...
    this.cars.map((car) => this.drive({ dt, player, car, speedScale }));
  };

  drive = ({ dt, player, car, speedScale }) => {
    const { hitBy } = player;
    const offset = 11;

    car.mesh.position.x += car.speed * speedScale;
