...
const TRAIN_WARNING_ROW_DISTANCE = 2;
...
export default class RailRoad extends Object3D {
  ...
  update = (dt, player, speedScale = 1) => {
    ...
    this.drive({ dt, player, speedScale });
  };

  drive = ({ dt, player, speedScale }) => {
    const { hitByTrain, moving } = player;
    const { train } = this;
    const offset = 22 * 5;

    train.mesh.position.x += train.speed * speedScale;

    if (train.mesh.position.x > offset && train.speed > 0) {
      train.mesh.position.x = -offset;
      this.startRingingLight();
      this.playWarningIfPlayerIsClose(player);
      if (train === hitByTrain) {
  ...
  playWarningIfPlayerIsClose = (player) => {
    if (!player.isAlive) {
      return;
    }

    const rowDistance = Math.abs(Math.round(player.position.z) - this.position.z);
    if (rowDistance <= TRAIN_WARNING_ROW_DISTANCE) {
      ProceduralAudio.playTrainWarning();
    }
  };
  ...
  trainShouldCheckCollision = ({ player }) => {
    const { train } = this;
    if (
      Math.round(player.position.z) === this.position.z &&
      player.isAlive &&
      !player.isInvincible
    ) {
